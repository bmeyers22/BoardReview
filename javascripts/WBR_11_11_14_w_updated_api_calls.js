...
function logoutAll(){
  ...
  }else{window.location.replace("/index.php?title=Special:UserLogout&returnto="+wgPageName)}
}
...
function addLoginInformation(){
  ...
  if(!$.isEmptyObject(wgUserName)){
    ...
    window.userName=mw.user.getName()
  ...
  if(window.fbUserID!=null & !loginChosen){
    ...
    window.userName=window.fbUserID;
    $("#chooseQBank").append(logoutHTML);
    $('#logoutDiv').click(logoutAll);
  }else{  
...
function selectExamOptions(event){
  ...
  var examOptionsHtml= "<div id='examOptions' style='display:none'>"+
      "<div id=topOptionBar>" + "<div id=topOptionBarTitle>Set up your exam</div>" +
          ...
          "<div style='color:#222; padding:10px;display:inline-block;text-align:left'>"+
              ...
              "<form id='myForm'>"+
                  "<input type='radio' name='SeenOptions' value='true' id='allButton' class='css-checkbox' />"+
                  ...
                  "<br />"+
                  "<input type='radio' name='SeenOptions' value='false' id='unseenButton' class='css-checkbox' />"+
  ...
  $('#newExamBigButton').click(function () {
    ...
    $('#newExamArrow').css({ opacity: 1 });
      $('#newExamArrow').show()
      $('#myExamsArrow').css({ opacity: 0 });
      $('#examOptions').show();
  ...
  $('#myExamBigButton').click(function () {
    $('#selectMenuPrompt').hide()
    $('#myExamsArrow').css({ opacity: 1 });
      $('#myExamsArrow').show();
      $('#newExamArrow').css({opacity:0});
      $('#examOptions').hide();
...
function deleteExam(deleteExamID){
  var api = new mw.Api();
  api.get({
    action : 'exams',
    subAction:'delete',
    exam: JSON.stringify({
      id:window.deleteExamID,
      }),
    format : 'json'
  }).done(function(data){
    console.log(data)
...
function addToNavBar() {
  ...
  window.exam.examid=window.examinfo.id;
  if (window.exam.timer) {
...
function addRatyRating() {
  ...
  getRating(window.exam.examQuestions[exam.currentQuestion].title, window.exam);
}
function addFirstResponses() {
  ...
  api.get({
    ...
    question : window.exam.examQuestions[exam.currentQuestion].title,
    format : 'json'
  })
  .done(function (data) {
...
/*  var api = new mw.Api();
  ...
  api.get({
    ...
    subAction:'save',
    examQuestion: JSON.stringify({
      username: window.userName,
      title: window.exam.examQuestions[window.exam.currentQuestion].title,
      selected: window.exam.examQuestions[window.exam.currentQuestion].selectedAnswer,
      exam_id: window.exam.examid,
      }),
    format : 'json'
  })
  .done(function (data) {
...
function submitRating2(ratingType, score, page) {
  ...
  if (ratingType === "difficulty") {
    ...
    api.get({
      ...
      ok : function (res) {console.log(res)}
    });
  } else if (ratingType === "yield") {
    var api = new mw.Api();
    api.get({
...
$(document).ready(function(){
  $.getScript("http://rawgit.com/jashkenas/underscore/master/underscore-min.js", function(){
