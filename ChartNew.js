...
window.Chart = function(context) {
          ...
          if (!(typeof(data[i].value) == 'undefined')) {
            ...
            if (config.inGraphDataShow) {
              ...
              var dispString = tmplbis(config.inGraphDataTmpl, {
                ...
                v10: fmtChartJS(config, calculateOffset(config, 1 * data[i].value, calculatedScale, scaleHop), config.fmtV10),
          ...
          for (var j = 0; j < data.datasets[i].data.length; j++) {
            if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
              ...
              var dispString = tmplbis(config.inGraphDataTmpl, {
                ...
                v9: fmtChartJS(config, midPosX + Math.cos(config.startAngle * Math.PI / 180 - j * rotationDegree) * calculateOffset(config, data.datasets[i].data[j], calculatedScale, scaleHop), config.fmtV9),
                v10: fmtChartJS(config, midPosY - Math.sin(config.startAngle * Math.PI / 180 - j * rotationDegree) * calculateOffset(config, data.datasets[i].data[j], calculatedScale, scaleHop), config.fmtV10),
  ...
  var Line = function(data, config, ctx) {
    ...
    if (mxlgt == 1) {
      ...
      for (var i = 0; i < data.datasets.length; i++) {
        if (typeof(data.datasets[i].data[0] != "undefined")) data.datasets[i].data = [, data.datasets[i].data[0], ];
      }
    }
            ...
            if (animPc >= 1) {
              ...
              if (config.inGraphDataShow) {
                ...
                var dispString = tmplbis(config.inGraphDataTmpl, {
                  ...
                  v7: fmtChartJS(config, totvalue[j], config.fmtV7),
                  v8: roundToWithThousands(config, fmtChartJS(config, 100 * data.datasets[i].data[j] / totvalue[j], config.fmtV8), config.roundPct),
                  v9: fmtChartJS(config, yAxisPosX, config.fmtV9),
                  v10: fmtChartJS(config, xAxisPosY - (calculateOffset(config, data.datasets[i].data[j], calculatedScale, scaleHop)), config.fmtV10),
                  v11: fmtChartJS(config, i, config.fmtV11),
                  v12: fmtChartJS(config, j, config.fmtV12)
                });
      ...
      for (var i = 0; i < calculatedScale.steps; i++) {
        if (labelTemplateString) {
          calculatedScale.labels.push(tmpl(labelTemplateString, {
            value: fmtChartJS(config, 1 * ((config.scaleStartValue + (config.scaleStepWidth * (i + 1))).toFixed(getDecimalPlaces(config.scaleStepWidth))), config.fmtYLabel)
          }));
        }
      }
      msr = setMeasures(data, config, ctx, height, width, calculatedScale.labels, true, false, true, true, true);
    ...
    function drawBars(animPc) {
      ...
      for (var i = 0; i < data.datasets.length; i++) {
        ...
        for (var j = 0; j < data.datasets[i].data.length; j++) {
          ...
          if (typeof(data.datasets[i].fillColor) == "string") {
            ctx.fillStyle = data.datasets[i].fillColor;
          } else if (typeof(data.datasets[i].fillColor[0]) == "string") {
            ctx.fillStyle = data.datasets[i].fillColor[Min([data.datasets[i].fillColor.length, j])];
          }

          if (typeof(data.datasets[i].strokeColor) == "string") {
            ctx.strokeStyle = data.datasets[i].strokeColor;
          } else if (typeof(data.datasets[i].strokeColor[0]) == "string") {
            ctx.strokeStyle = data.datasets[i].strokeColor[Min([data.datasets[i].strokeColor.length, j])];
          }
          ...
          for (var j = 0; j < data.datasets[i].data.length; j++) {
            ...
            if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
              ...
              var dispString = tmplbis(config.inGraphDataTmpl, {
                ...
                v10: fmtChartJS(config, xAxisPosY - calculateOffset(config, data.datasets[i].data[j], calculatedScale, scaleHop) + (config.barStrokeWidth / 2), config.fmtV10),
                v11: fmtChartJS(config, i, config.fmtV11),
                v12: fmtChartJS(config, j, config.fmtV12)
              });
              ...
              if (config.inGraphDataYPosition == 1) {
                yPos = xAxisPosY - yStart[j] - config.inGraphDataPaddingY;
              } else if (config.inGraphDataYPosition == 2) {
                yPos = xAxisPosY - (calculateOffset(config, (yFpt[j] >= 0) * calculatedScale.graphMin + 1 * data.datasets[i].data[j], calculatedScale, scaleHop) + (config.barStrokeWidth / 2)) / 2 - yStart[j] - config.inGraphDataPaddingY;
              } else if (config.inGraphDataYPosition == 3) {
                yPos = xAxisPosY - calculateOffset(config, (yFpt[j] >= 0) * calculatedScale.graphMin + 1 * data.datasets[i].data[j], calculatedScale, scaleHop) + (config.barStrokeWidth / 2) - yStart[j] - config.inGraphDataPaddingY;
              }
        ...
        for (var i = 0; i < data.datasets.length; i++) {
          ...
          for (var j = 0; j < data.datasets[i].data.length; j++) {
            ...
            if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
              ...
              if (config.inGraphDataXPosition == 1) {
                xPos = yAxisPosX + yStart[j] + 1 + config.inGraphDataPaddingX;
              } else if (config.inGraphDataXPosition == 2) {
                xPos = yAxisPosX + yStart[j] + (HorizontalCalculateOffset((yFpt[j] >= 0) * calculatedScale.graphMin + 1 * data.datasets[i].data[j], calculatedScale, valueHop) + (config.barStrokeWidth / 2)) / 2 + config.inGraphDataPaddingX;
              } else if (config.inGraphDataXPosition == 3) {
                xPos = yAxisPosX + yStart[j] + HorizontalCalculateOffset((yFpt[j] >= 0) * calculatedScale.graphMin + 1 * data.datasets[i].data[j], calculatedScale, valueHop) + (config.barStrokeWidth / 2) + config.inGraphDataPaddingX;
              }
              if (config.inGraphDataYPosition == 1) {
          ...
          for (var j = 0; j < data.datasets[i].data.length; j++) {
            ...
            if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
              ...
              if (config.inGraphDataYPosition == 1) {
                ...
                yPos = xAxisPosY - (calculateOffset(config, 1 * data.datasets[i].data[j], calculatedScale, scaleHop) + (config.barStrokeWidth / 2)) / 2 - config.inGraphDataPaddingY;
              } else if (config.inGraphDataYPosition == 3) {
                yPos = xAxisPosY - calculateOffset(config, 1 * data.datasets[i].data[j], calculatedScale, scaleHop) + (config.barStrokeWidth / 2) - config.inGraphDataPaddingY;
              }
          ...
          for (var j = 0; j < data.datasets[i].data.length; j++) {
            if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
              ...
              if (config.inGraphDataXPosition == 1) {
                ...
                xPos = yAxisPosX + (calculateOffset(config, 1 * data.datasets[i].data[j], calculatedScale, valueHop) + (config.barStrokeWidth / 2)) / 2 + config.inGraphDataPaddingX;
              } else if (config.inGraphDataXPosition == 3) {
                xPos = yAxisPosX + calculateOffset(config, 1 * data.datasets[i].data[j], calculatedScale, valueHop) + (config.barStrokeWidth / 2) + config.inGraphDataPaddingX;
              }
